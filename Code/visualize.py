...
myFile = open(os.path.join("Data","expectimax2vgenetic.txt"))
white_score = []
black_score = []
white_wins = []
black_wins = []
...
plt.plot(games, white_score, color='blue', label='Expectimax Score', linewidth=3)
plt.plot(games, black_score, color='red', label='Genetic Score', linewidth=3)
plt.plot(games, white_wins, color='purple', label='Expectimax Wins', linewidth=3)
plt.plot(games, black_wins, color='orange', label='Genetic Wins', linewidth=3)
# Add labels, legend, and title
plt.xlabel('Game Number')
plt.ylabel('Cumulative Totals')
plt.title('Cumulative Scores and Wins Across 231 Cubeful First-to-25 Matches')
plt.legend()
plt.grid(True)
plt.show()